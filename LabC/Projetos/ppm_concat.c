...
// remover quaisquer comentarios da 2 imagem
  ...
  if(first.compr < second.compr){
    ...
    fprintf(end, "%s\n%d %d\n%d\n",first.type,maxlarg,first.compr,first.pixelmax);

  PIXEL **array;
  array = malloc( first.compr * sizeof(struct Pixel*) );
    if(array == NULL){
    ...
    for(int i=0;i<first.compr;i++){
      ...
      for(int k=0,l=j;k<second.larg;l++,k++){
        array[i][l].red = s[i][k].red;
        array[i][l].green = s[i][k].green;
        array[i][l].blue = s[i][k].blue;
      }
    }

    for(int i=0;i<first.compr;i++)
      for(int j=0;j<maxlarg;j++){
