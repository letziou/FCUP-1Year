...
void Horizontal(PIXEL **array, int rows, int cols,int bar, int red, int green, int blue, int pos);
...
int main(int argc, char* argv[]) {
    ...
    array = malloc(m.compr * sizeof(struct Pixel*));
      ...
      int space = bar;
       ...
       for(int i=0;i<m.compr;i++){
         if(i==space){
           Horizontal(array,m.compr,m.larg,bar,red,green,blue,i);
           space += (2*bar);
        }
      }
